...
- **Settings UI** for:
  - Parent email (Ex. tinyhuman1@gmail.com)
...
- **Background monitoring**:
  ...
  - privacy is ensured and only the parent email provided will be able to view screenshots
- **Warning popup**:
...
- **Notifications**: If enabled, sends email to parent with warning summary. Screenshot saved locally.
...
- **Email image alerts:** When an email is configured, a
  suspicious-event screenshot is briefly uploaded to a free temporary public image host
  (tmpfiles.org, with 0x0.st as fallback) so it can be attached to the email sent to the
  parent. This upload happens **only** for events that trigger an alert. If the upload
  fails, the app automatically falls back to a text-only email. If you do not want any image
